...
// 선택 구간을 MP3 Blob으로 (다운로드하지 않음)
// 저장 다이얼로그(SaveSentenceDialog)가 같은 클립을 파일로 떨구지 않고 그대로 업로드하려고 쓴다.
export async function extractRegionToMp3Blob(mediaUrl: string, startSec: number, endSec: number, kbps = 192): Promise<Blob> {
  const { channels, sampleRate } = await decodeRegion(mediaUrl, startSec, endSec);
  return encodeMp3(channels, sampleRate, kbps);
}

// 선택 구간을 WAV Blob으로 (다운로드하지 않음)
export async function extractRegionToWavBlob(mediaUrl: string, startSec: number, endSec: number): Promise<Blob> {
  const { channels, sampleRate } = await decodeRegion(mediaUrl, startSec, endSec);
...
export async function extractRegionToMp3(mediaUrl: string, startSec: number, endSec: number, fileName?: string | null, kbps = 192): Promise<void> {
  const blob = await extractRegionToMp3Blob(mediaUrl, startSec, endSec, kbps);
...
export async function extractRegionToWav(mediaUrl: string, startSec: number, endSec: number, fileName?: string | null): Promise<void> {
  const blob = await extractRegionToWavBlob(mediaUrl, startSec, endSec);
