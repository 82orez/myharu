...
- **로그인**: 게이미피케이션 대시보드 (전체·오늘 진행률 원형 차트 2개 + XP/스트릭 2카드 + 월간 학습 달력 + 액션 카드) + 신규 사용자 온보딩 안내
...
- 음성/텍스트 결과는 축약형 확장 + 단어 유사도(80% 이상)로 관대하게 판정

### 게이미피케이션

- **XP (경험치)**: 정답 10 XP, 도전 2 XP. 대시보드에서 누적 XP 확인. 같은 문장 반복 정답도 매번 누적
- **연속 학습 스트릭**: 매일 첫 학습 시 스트릭 +1, 건너뛰면 리셋. 최고 기록 추적
