...
- **재생 순서: `src` 대입 → `load()` → `waitUntilReady`(canplay/canplaythrough, `READY_TIMEOUT_MS`=2s 타임아웃 시 그냥 재생) → `currentTime=0` → `play()`.** 대기 중 다음 재생이 끼어들면 `requestRef` 순번 비교로 이전 요청을 폐기한다(`stop()`·언마운트도 순번을 올려 무효화).
- **iOS 앞부분 잘림 → 무음 pre-roll (걷어내지 말 것)**: 아이폰에서 "듣기" 첫 단어가 잘린다는 제보를 **실측으로 확인** — 문장 61개의 lead-in(첫 소리까지의 무음)을 디코딩해 재니 **47~70ms(34개)=잘리는 카드 / 142~558ms(27개)=정상 카드**로 정확히 갈렸다. 파일·코덱(전부 mp3 48kHz)도, 볼륨 보정(Web Audio) 경로도 원인이 아니고 **iOS가 재생 시작 직후 ~0.1초 출력을 삼키는 것**(여백이 길면 티가 안 날 뿐). → `isIOS()`일 때만 소리를 끈 채(plain=`muted`, 증폭=GainNode 0) `IOS_PREROLL_MS`(400ms) 흘려보낸 뒤 `currentTime=0`으로 되감고 소리를 켠다. 첫 단어가 여전히 잘리면 이 상수만 올린다. ⚠️ pre-roll 타이밍을 `await el.play()`로 잡지 말 것 — 프로미스가 resolve되지 않는 상황에서 **음소거로 굳는다**(타이머로 독립시킬 것). "버퍼링 문제"로 오해해 준비 대기만 넣었던 앞선 수정으로는 해결되지 않았다.
- **첫 사용자 입력에 무음 워밍업**(0.05s 무음 WAV data URI)으로 iOS 오디오 세션을 미리 깨운다. ⚠️ 워밍업은 **반드시 별도 엘리먼트**로 — 재생용 `nodes.plain`을 쓰면 "듣기" 탭이 곧 첫 pointerdown이라 워밍업 정리(pause/src 제거)가 **방금 시작된 진짜 재생을 죽인다**(구현 중 실제로 만든 버그). 음소거 복구는 타이머 폴백 필수(`WARMUP_RESTORE_MS`).
