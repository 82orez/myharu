...
- **학습** (`/learn/review`): "문장 목록"(기본 탭) / "퀴즈" 탭 전환(`ReviewTabs`). 문장 목록(`ReviewClient`)에서 듣기(재생 상태 추적, 재생 중 다른 버튼 비활성화)/말하기(개별 문장 스피킹 테스트, 결과는 toast로 표시 — XP/스트릭 미반영)/정답 보기/즐겨찾기/편집(텍스트 수정 + 선택적 음성 재생성)/삭제 지원. 편집 시 `generateAudio`(input/actions.ts) 재활용. 퀴즈(`QuizView`)는 듀오링고 스타일 한 문제씩(`useReducer` 상태 머신: ready → question → listening → result → summary). 프로그레스 바, 정답 시 초록 글로우 + "+10 XP" 플로팅, 오답 시 카드 흔들림 + 정답 표시. 세션 완료 후 `SessionSummary`(정확도 원형 프로그레스, 통계 그리드).
