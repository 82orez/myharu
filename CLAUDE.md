...
**My Haru** — 나의 하루를 기록하고 관리하는 개인 일정/습관/영어 학습 서비스입니다. Next.js 16 + Supabase Auth + shadcn/ui + Pretendard 기반. App Router + React 19 + Tailwind v4 + Turbopack(기본 번들러) 조합.
...
- 포함 컴포넌트(`src/components/ui/`): `button`, `card`, `input`, `label`, `alert-dialog`. 추가는 `npx shadcn@latest add <name>`.
...
### 브랜드 컬러
브랜드 색이 필요하면 `globals.css`의 `:root`에 `--brand: ...;` 추가 후 `@theme inline`에서 `--color-brand: var(--brand);` 매핑, 필요 시 `button.tsx`의 `cva`에 `brand` variant 추가.
...
│   ├── learn/                 # InputForm (문장 입력 폼), ReviewClient (복습 UI + 음성 인식)
