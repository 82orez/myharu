...
### 구조
- **문장 입력** (`/learn/input`): 영어 문장 + 한국어 뜻 입력 → OpenAI TTS로 mp3 생성 → Supabase Storage 업로드 + `sentences` 테이블 저장. 글자수 카운터, 예시 힌트, 최근 저장 표시.
- **퀴즈 복습** (`/learn/review`): 듀오링고 스타일 한 문제씩 퀴즈. `QuizView` 컴포넌트(`useReducer` 상태 머신: ready → question → listening → result → summary). 프로그레스 바, 정답 시 초록 글로우 + "+10 XP" 플로팅, 오답 시 카드 흔들림 + 정답 표시. 세션 완료 후 `SessionSummary`(정확도 원형 프로그레스, 통계 그리드).
...
- **`sentences`**: id, user_id, english_text, korean_text, audio_path, created_at. RLS: `user_id = auth.uid()`. Storage `tts-audio` 버킷도 동일 RLS.
...
│   ├── Navbar.tsx             # "use client", 프로스티드 글래스, 모바일 로그인 시 햄버거 숨김 (BottomNav 대체)
