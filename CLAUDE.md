...
- **할 일** (`/todo`, 서버 컴포넌트 + `components/todo/TodoClient.tsx`): 학습과 무관한 범용 할 일. `(learn)` 그룹 안이라 경로엔 그룹명이 빠지고 **로그인 가드는 그룹 레이아웃이 담당**한다. 서버 액션 `todo/actions.ts`(`getTodos`/`addTodo`/`updateTodo`/`toggleTodo`/`moveTodo`/`deleteTodo`/`clearCompletedTodos`), 정렬·필터·날짜 로직은 **`lib/todo.ts` 공용**(`sortTodos`·`filterTodos`·`nextDueDate`·`todayKST` — 컴포넌트에 다시 구현하지 말 것). 목록은 낙관적 갱신 후 실패 시 롤백(`ReviewClient` 패턴). 우선순위 배지 클래스는 `PRIORITIES`에 **전체 문자열로** 나열(Tailwind 스캔). ⚠️ **반복 할 일을 완료하면 새 행을 만들지 않고 같은 행의 `due_date`를 다음 주기로 밀고 `is_done=false`를 유지**한다(완료 목록이 반복 항목으로 차지 않게 — 인스턴스 생성 방식으로 바꾸지 말 것). ⚠️ 수동 정렬은 `position double precision`에 **이웃 두 값의 중간값**만 써 한 행만 갱신한다(정수로 바꾸면 뒤 항목 전체를 다시 써야 한다). 모바일에서 HTML5 드래그가 안 되므로 **드래그 + ↑↓ 버튼을 함께** 제공한다(둘 중 하나만 남기지 말 것). 날짜 판정은 모두 KST.
- **메모** (`/memo`, 서버 컴포넌트 + `components/memo/{MemoClient,MemoEditorDialog}.tsx`): Google Keep 스타일 메모(제목·본문·12색·고정·보관함·검색). 서버 액션 `memo/actions.ts`(`getMemos`/`addMemo`/`updateMemo`/`setMemoPinned`/`setMemoArchived`/`deleteMemo`), 정렬·필터·색 팔레트는 **`lib/memo.ts` 공용**(`sortMemos`=고정 먼저→`updated_at` 내림차순, `filterMemos`=뷰+검색어). 그리드는 **CSS 컬럼 메이슨리**(`columns-1 sm:columns-2 lg:columns-3` + 카드 `break-inside-avoid`), `메모` 뷰는 고정/기타 두 섹션. ⚠️ `MEMO_COLORS`의 클래스는 **전체 문자열로 나열**(Tailwind 스캔 — `tag-color.ts`와 같은 이유), 라이트 전용이라 `dark:` 금지. ⚠️ 카드 전체가 편집 모달 트리거라 카드 안 액션(고정·색상·보관·삭제)은 **`stopPropagation` 필수**. 편집 모달은 **닫을 때 변경분만 저장**(변경 없으면 서버 미호출, 빈 메모가 되면 저장하지 않고 원본 유지). `updated_at`은 트리거가 없어 **서버 액션에서 직접 갱신**한다(정렬 기준이라 빠뜨리면 순서가 안 바뀐다). 보관하면 고정은 자동 해제.
- **설정** (`/settings`, 서버 컴포넌트 + `components/settings/*`): 섹션 카드 4종 — 계정(이메일·로그아웃) / 학습(스피킹 채점 난이도 `SpeechStrictField`는 **클릭 즉시 저장**, 하루 목표 `DailyGoalField`는 숫자 입력 + "저장" 버튼(값이 바뀌고 유효할 때만 활성, 낙관적 갱신 없음), D-day `DdayField`(이름 + 날짜/기간 입력 + "저장" — 날짜를 비우고 저장하면 해제), 효과음 `FeedbackSoundField`·리스닝 자동 재생 지연 `AutoPlayDelayField`(0/1/2/3초, `lib/quiz-autoplay.ts`)·음성 인식 모델 `SttModelField`(`lib/stt-models.ts` — Repeater 저장 다이얼로그와 같은 `myharu:stt-model` 값을 공유)는 **localStorage 기기별 설정**이라 서버 액션 없이 즉시 저장, 등록 문장 수는 읽기 전용) / 태그 관리(`TagManagerCard`→`TagManager`) / 데이터 관리(`ResetDataButton` 2종 + `DeleteAllSentences`). **연습 데이터 초기화 2종**(`ResetDataButton`, `kind`로 분기 — 둘 다 "초기화" 타이핑 확인): ① `연습 횟수 초기화`=`resetPracticeCounts`(sentences의 `speech/text/listen_count`만 0 → 카드 횟수·`미연습` 배지·홈 연습횟수 합계만 리셋, **달력·오늘 진도는 유지**) ② `학습 기록 초기화`=`resetPracticeHistory`(`practice_results` 전체 삭제 → **오늘 진도·달력만 리셋**, 카드 카운터는 유지). ⚠️ 둘은 서로 독립이고 **문장·음성 파일은 건드리지 않는다**. 표시 수치는 페이지의 `Promise.all`에서 `fetchPracticeCountTotal`·`practice_results` count로 함께 조회(직렬 추가 금지). **전체 문장 삭제** `deleteAllSentences`(`settings/actions.ts`)는 sentences 삭제 + Storage 100개씩 정리 — ⚠️ `practice_results` FK cascade로 **연습 기록도 함께 사라져** 오늘 진도·달력·연습횟수 합계가 초기화되므로 AlertDialog에서 "삭제" 타이핑 확인을 받는다. 장기 목표(총량/기간/완주선) 개념 없음.
- **D-day** (`/settings`에서 지정 → 모든 페이지 Navbar 우측 배지): `user_stats.dday_label`+`dday_date`. 계산·검증은 **`lib/dday.ts` 하나**(디렉티브 없는 순수 모듈 — `MAX_DDAY_LABEL`=20·`isValidDdayDate`·`ddayDiff`·`formatDday`·`dateAfterDays`·`DDAY_PERIOD_PRESETS`(20/50/100/200/300)·`MIN|MAX_DDAY_PERIOD`(1~3650)). **목표일 지정은 입력 방법 2가지** — `날짜`(달력) / `기간`(숫자 입력 + 프리셋 칩, `dateAfterDays`로 오늘+N일 계산 + `→ 날짜 · D-N` 미리보기). ⚠️ **저장 값은 `dday_date` 날짜 하나뿐**이고 `mode`는 컴포넌트 로컬 state다(서버·localStorage 어디에도 남기지 않음) — 기간을 별도 컬럼으로 저장하려 하지 말 것. ⚠️ 불변식 **`ddayDiff(dateAfterDays(N)) === N`**(오늘 미포함) — 오늘 포함 방식으로 바꾸면 "20일"을 넣었는데 `D-19`가 뜬다. ⚠️ 기간 프리필(`switchToPeriod`)·미리보기를 `useState` 초기값이나 무조건 렌더로 올리지 말 것 — `todayKST()`를 읽어 **SSR/하이드레이션 불일치**가 생긴다. **해제는 두 모드 대칭** — 날짜/기간 입력을 비우고 저장하면 `setDday(…, null)`로 해제된다(기간 모드는 빈 값일 때 `→ 저장하면 D-day 해제` 안내를 띄운다). ⚠️ 한쪽만 막지 말 것. 남은 일수는 `nextDueDate`와 같이 **`T12:00:00Z` 정오 기준**으로 빼서 서머타임·타임존 경계에 하루가 밀리지 않게 한다. 표시는 `D-42`/`D-DAY`/`D+3`(지난 뒤), 지난 경우만 주황(`accent-orange`) — ⚠️ 색 클래스는 삼항으로 **전체 문자열 나열**(Tailwind 스캔). `todayKST`는 `lib/todo.ts` 것을 재사용(`gamification.ts` 사본은 `"server-only"`라 클라 불가) — 세 번째 사본을 만들지 말 것. **날짜가 NULL이면 배지를 아예 렌더하지 않는다**(비로그인도 동일). ⚠️ 서버 액션 `setDday`는 `revalidatePath("/settings")` + **`revalidatePath("/", "layout")`** — 배지가 루트 레이아웃에 있어 page 단위 revalidate로는 다른 경로의 Navbar가 갱신되지 않는다. ⚠️ `layout.tsx`의 조회는 `fetchUserStats`(`select("*")` + 없으면 upsert)가 아니라 **`select("dday_label, dday_date")` + `maybeSingle()`** — 모든 페이지 렌더마다 도는 자리라 최소화한다. ⚠️ `DdayField`의 `<form noValidate>`를 빼지 말 것 — 날짜를 일부만 지우면(연도 segment만 삭제 등) `validity.badInput`이 서서 **네이티브 검증이 submit을 통째로 막아 "저장"이 눌려도 아무 일이 없다**(실제로 겪음). 이 상태의 `value`는 `""`라 해제로 처리하면 된다.
...
5개 테이블. RLS는 모두 `user_id = auth.uid()`.
...
- **`user_stats`**: user_id(PK), `tag_presets text[]`, `personal_message text`(기본 `''`), `speech_strict boolean`(기본 false, 스피킹 채점 난이도), `dday_label text`(기본 `''`, **CHECK ≤20자** — `MAX_DDAY_LABEL`과 같은 값 유지)·`dday_date date`(**nullable** = D-day 미설정), `daily_goal int`(기본 1000, **CHECK 1~10000** — UPDATE RLS가 소유자만 검사해 클라가 임의 값을 쓸 수 있으므로 서버 액션 검증만으로 부족), created_at. 신규 가입 시 `handle_new_user_stats` 트리거로 자동 생성.
- **`memos`**: id, user_id, `title`(≤200자)·`content`(≤10000자, 둘 다 빈 메모는 앱에서 저장 안 함), `color`(12색 CHECK), `is_pinned`, `is_archived`, created_at, `updated_at`. RLS 4종.
- **`todos`**: id, user_id, `title`(CHECK 1~200자), `note`(기본 `''`), `is_done`, `priority`(`low|normal|high` CHECK), `due_date date`(nullable), `repeat`(`none|daily|weekly|monthly` CHECK), `position double precision`(수동 정렬 키), created_at, `completed_at`. RLS **select/insert/update/delete 4종**(완료 토글·수정 때문에 update 필수).
...
│   ├── (learn)/               # learn/input·review·quiz·player + settings + todo·memo(각 actions.ts 포함) (+ gamification-actions.ts, loading.tsx, error.tsx)
...
│   ├── memo/                  # MemoClient/MemoEditorDialog(Keep 스타일 메모 — 색상·고정·보관·검색)
│   ├── todo/                  # TodoClient(할 일 목록 — 추가·완료·인라인 수정·메모·우선순위·마감일·반복·수동 정렬)
│   ├── ui/                    # shadcn
│   ├── settings/              # SpeechStrictField(즉시 저장)/DailyGoalField·DdayField(입력+저장)/FeedbackSoundField·AutoPlayDelayField·SttModelField(localStorage)/TagManagerCard/ResetDataButton/DeleteAllSentences
...
│   ├── DdayBadge.tsx          # Navbar 우측 D-day 배지(props 전용 순수 컴포넌트)
...
├── lib/{utils,origin,email,rate-limit,normalize-text,openai,gamification,tags,tag-color,tts-voices,settings-config,audio-loudness,audio-formats,audio-upload,feedback-sound,speech-recognition,sentence-number,sentence-filter,quiz-autoplay,dday,stt-models,stt-client,todo,memo}.ts
