...
- **재생 순서: `src` 대입 → `load()` → `waitUntilReady`(canplay/canplaythrough, `READY_TIMEOUT_MS`=2s 타임아웃 시 그냥 재생) → `currentTime=0` → `play()`.** ⚠️ `src` 대입 직후 바로 `play()` 하도록 되돌리지 말 것 — **iOS Safari에서 앞 1초가 유실된다**(아이폰 제보로 확인. 편집 폼 미리듣기는 렌더 시 이미 버퍼링돼 멀쩡했던 게 단서). 대기 중 다음 재생이 끼어들면 `requestRef` 순번 비교로 이전 요청을 폐기한다.
