...
- **학습**: **별도 라우트 2개** — 문장 목록(`/learn/review`)과 퀴즈(`/learn/quiz`). 두 페이지 상단에 공용 `LearnModeTabs`(`usePathname` 기반 `<Link>` 탭, 활성 강조)로 전환. **학습 인정은 문장 목록에서만**. (기존 nav 링크는 모두 `/learn/review`=문장 목록을 가리킴.)
  ...
  - **퀴즈** (`/learn/quiz`, `QuizView`): 한 문제씩(`useReducer` 상태머신 `ready→question→listening→result→summary`). 스피킹/텍스트 모드. **세션 타입**(`quizType` 컴포넌트 state, ready 화면에서 택1): `translate`(한국어 뜻 보고 영→말하기/쓰기) / `listening`(오디오 듣고 따라 말하기 — 한국어·영어 **모두 숨김**, **문제 카드 클릭으로 오디오 재생**(듣기 버튼 없음, 재생/수음 중엔 비활성), **말하기만**, speech로 집계). ⚠️ `quizType==='listening'`(세션 타입)과 reducer `phase==='listening'`(마이크 수음 중)은 별개. **`recordPracticeResult` 미호출(점수·XP·암기 무관)**, 단 **정답 시** `incrementPracticeCount(sentenceId, mode)`로 정답 횟수만 누적. 요약은 정확도만. 페이지는 `getSentences`+`getUserStats` fetch. 진행율 바·카운터는 `currentIndex` 기준(현재 문제 = `(currentIndex+1)/total`, 오답·재시도 시 증가 안 함). `answers`는 문제당 1개(`currentIndex`로 덮어써 중복 방지). 오답 결과에서 "다음"은 `AlertDialog` 확인(오답 확정 경고) 후 이동. 듣기 오디오 재생 중(`isPlaying`)엔 모든 액션 버튼 비활성(듣기 버튼은 스피너+"playing"), 문제 전환 시 재생 정지.
