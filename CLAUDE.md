...
- **학습 달력**: `fetchDailyMemorized`가 반환하는 `Record<날짜(YYYY-MM-DD), 신규암기수>`. 각 문장의 **최초 정답 KST 날짜**로 집계(이후 반복 정답은 미가산 → 일일 진도 분자 정의와 일치). 홈 대시보드의 `LearningCalendar`(월간 히트맵). 신규 암기 수 / 일일 목표(`dailyGoalDisplay`) 비율로 색 농도 레벨(0~4) 산출, 월 이동 가능(현재 월 이후로는 비활성), 오늘 날짜는 브랜드 링 강조.
