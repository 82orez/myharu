...
- **스트릭**: **"일일 목표를 달성한 날"의 연속 수**(듀오링고식). KST 기준. `recordPractice`에서 오늘 신규 암기 수(`fetchDailyProgress.completed`)가 그날의 일일 목표(장기 목표 있으면 동적 `dailyMinimum`, 없으면 `daily_goal`) 이상이면 그날을 카운트 — 하루 1회만 반영. `last_practice_date`는 **마지막으로 목표를 달성한 날**을 의미(미달인 날은 갱신 안 함). 달성일이 어제면 +1, 그 외면 1로 리셋, `longest_streak` 자동 추적. 단순 시도/오답·목표 미달은 스트릭에 미반영(XP만 누적). 표시 시 `effectiveStreak(stats)`로 보정 — `last_practice_date`가 오늘도 어제도 아니면 0(끊김)으로 표시. 홈(`page.tsx`)은 이 보정값을 배지·부제목·연속 스탯 카드에 사용.
...
- **장기 목표**: `fetchGoalProgress`가 반환하는 `GoalProgress` = `{ memorized, totalGoal, periodDays, startDate, daysElapsed, daysRemaining, dailyMinimum, percentage, isOnTrack }`. `dailyMinimum = ceil((totalGoal - memorized) / max(1, daysRemaining))`로 매 read 시 동적 재계산(남은 일수에서 차감). `isOnTrack = memorized * periodDays >= totalGoal * daysElapsed`. 홈 대시보드의 `GoalProgressCard`(**전체(목표 누적)·오늘(당일) 원형 차트 2개 나란히** — 내부 `Ring`/`RingBlock` 헬퍼 + 남은 일수/오늘 최소/페이스 상태 배너). 당일 데이터는 page에서 `dailyCompleted`/`dailyGoal`(=`dailyGoalDisplay`) prop으로 전달. 목표 미설정 시엔 "오늘" 링 + 목표 설정 프롬프트만 표시.
...
정규화 순서: 스마트 따옴표 통일 → 소문자 변환 → **축약형 확장** (약 45개, `"i'm"` → `"i am"` 등) → **구어 변형 표준화** (`VARIANTS`, `"okay"` → `"ok"`, `"gonna"` → `"going to"`, `"yeah"` → `"yes"` 등) → 구두점 제거 → 공백 정리. 변형 치환은 정답·입력 양쪽에 동일 적용되므로 거짓 오답만 줄임(대칭적).
